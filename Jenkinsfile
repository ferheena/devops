pipeline {
    ...
    stages {
        ...
        stage ('sonarAnyalysis') {
            steps {
                sh "/opt/sonarscanner/sonar-scanner-3.2.0.1227-linux/bin/sonar-scanner -Dsonar.host.url=http://34.93.49.84:9000 -Dsonar.projectName=saas12 -Dsonar.projectVersion=1.0 -Dsonar.projectKey=saas12 -Dsonar.sources=. -Dsonar.projectBaseDir=/var/lib/jenkins/workspace/saas12_master -Dsonar.python.coveragePlugin=cobertura -Dsonar.python.pylint=/usr/local/bin/pylint -Dsonar.python.pylint.reportPath=pylint-report.txt -Dsonar.python.coverage.reportPath=coverage-reports/*coverage-*.xml"
            }
        }
